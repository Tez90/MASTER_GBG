 ...
 * <ul>
 * <li> if mode=0: {@link EvaluatorPoker#evaluateAgent1(PlayAgent,PlayAgent,GameBoard)} (competition against RandomAgent) or
 * <li> if mode=1: {@link EvaluatorPoker#evaluateAgent1(PlayAgent,PlayAgent,GameBoard)} (competition against MaxNAgent) or
 * <li> if mode=2: {@link EvaluatorPoker#evaluateAgent2(PlayAgent,PlayAgent,GameBoard)} (competition against MaxNAgent from different start positions).
 * </ul>  
...
public class EvaluatorPoker extends Evaluator {
	...
	public EvaluatorPoker(PlayAgent e_PlayAgent, GameBoard gb, int stopEval) {
		super(e_PlayAgent, gb, 1, stopEval);
		initEvaluator(gb);
	}

	public EvaluatorPoker(PlayAgent e_PlayAgent, GameBoard gb, int stopEval, int mode) {
     ...
     */
	private ArrayList<StateObserverPoker>
	addAll1PlyStates(ArrayList<StateObserverPoker> diffStartList) {
    	StateObserverPoker s = (StateObserverPoker) m_gb.getDefaultStartState();
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {         	
				ACTIONS a = new ACTIONS(i * 3 + j);
	        	StateObserverPoker s_copy = s.copy();
	        	s_copy.advance(a);
	 ...
	 * {@link #EvaluatorPoker(PlayAgent, GameBoard, int, int)} [default mode=1].<p>
	 * 
	...
	public boolean evalAgent(PlayAgent playAgent) {
        ...
        if (diffStartList==null) {
        	diffStartList = new  ArrayList<StateObserverPoker>();
...
//		lastResult=0;
        ...
        int i=0;
        for (StateObserverPoker so : diffStartList) {
