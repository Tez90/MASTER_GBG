 ...
 * <ul>
 * <li> if mode=0: {@link EvaluatorPenney#evaluateAgent1(PlayAgent,PlayAgent,GameBoard)} (competition against RandomAgent) or
 * <li> if mode=1: {@link EvaluatorPenney#evaluateAgent1(PlayAgent,PlayAgent,GameBoard)} (competition against MaxNAgent) or
 * <li> if mode=2: {@link EvaluatorPenney#evaluateAgent2(PlayAgent,PlayAgent,GameBoard)} (competition against MaxNAgent from different start positions).
 * </ul>  
 * The value of mode is set in the constructor. Class Evaluator2 works also for featmode==3.
 */
public class EvaluatorPenney extends Evaluator {
	 ...
	 */
	protected static ArrayList<StateObserverPenney> diffStartList = null;
	
	public EvaluatorPenney(PlayAgent e_PlayAgent, GameBoard gb, int stopEval) {
		super(e_PlayAgent, gb, 1, stopEval);
		initEvaluator(gb);
	}

	public EvaluatorPenney(PlayAgent e_PlayAgent, GameBoard gb, int stopEval, int mode) {
		super(e_PlayAgent, gb, mode, stopEval);
		initEvaluator(gb);
	}

	public EvaluatorPenney(PlayAgent e_PlayAgent, GameBoard gb, int stopEval, int mode, int verbose) {
     ...
     */
	private ArrayList<StateObserverPenney>
	addAll1PlyStates(ArrayList<StateObserverPenney> diffStartList) {
    	StateObserverPenney s = (StateObserverPenney) m_gb.getDefaultStartState();
		for (int i = 0; i < 3; i++) {
			ACTIONS a = new ACTIONS(i);
			StateObserverPenney s_copy = s.copy();
			s_copy.advance(a);
	 ...
	 * {@link #EvaluatorPenney(PlayAgent, GameBoard, int, int)} [default mode=1].<p>
	 * 
	...
	public boolean evalAgent(PlayAgent playAgent) {
        ...
        if (diffStartList==null) {
        	diffStartList = new  ArrayList<StateObserverPenney>();
...
//		lastResult=0;
        ...
        int i=0;
        for (StateObserverPenney so : diffStartList) {
